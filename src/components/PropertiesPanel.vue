<template>
    <div v-if="node">
        <div class="rounded-md">
            <Accordion class="pr-2" title="Properties">
            <div class="flex">
                <form-label class="ml-2">object type:</form-label>
                <form-label class="ml-2">{{node.type}}</form-label>
            </div>
                ...
                <div v-if="node.type !== 'page'">
                    ...
                    <form-select name="indicator type" type="number" min="0" max="1" v-model="node.hasp_type"
                        v-if="node.hasp_type !== undefined" v-on:change="node.adjust()">
                        <option value="0">clock-wise</option>
                        <option value="1">counter-clock-wise</option>
                    </form-select>
                ...
                <form-input name="border-radius" type="number" min="0" v-model="node.radius"
                    v-if="node.radius !== undefined" v-on:input="node.setRadius($event.target.value)" />
                ...
                <form-select name="arc type" v-model="node.arc_type" v-if="node.arc_type !== undefined"
                    v-on:change="node.setArcType($event.target.value)">
                    <option value="0">normal</option>
                    <option value="1">symmetrical</option>
                    <option value="2">reverse</option>
                </form-select>

                <form-input name="start angle" type="number" min="0" v-model="node.start_angle"
                    v-if="node.start_angle !== undefined" v-on:input="node.setArcAngles($event.target.value)" />
            </Accordion>
            <Accordion class="pr-2" title="Data" v-if="node.val !== undefined">
                <form-input name="min value" type="number" min="0" v-model="node.min" v-if="node.min !== undefined"
                    v-on:input="node.adjust()" />
                <form-input name="max value" type="number" min="0" v-model="node.max" v-if="node.max !== undefined"
                    v-on:input="node.adjust()" />
                <form-input name="critical value" type="number" min="0" v-model="node.critical_value" v-if="node.critical_value !== undefined"
                    v-on:input="node.adjust()" />
                ...
                <div class="grid grid-cols-2 justify-items-end">
                    <template v-if="node.bg_color">
                        ...
                        <ColorPicker v-model:pureColor="node.bg_color" useType="pure" format="hex8" lang="En" shape="circle"
                            v-on:pureColorChange="node.setBgColor($event)" />
                    </template>
            ...
            <Accordion class="pr-2" title="Action" v-if="node.hasAction">
                ...
                <form-select name="Command" v-model="node.command">
                    <option :value=name v-for="(name) in node.getLayer().getObjectCommands(node.action)" :key=name>{{ name }}</option>
                </form-select>
